<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Data Sources</h2>
                <p class="text-sm text-gray-500 mt-1">{{ $site->name }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('app.sites.data.import', $site) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 transition">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <livewire:app.data.data-source-manager :site="$site" />
        </div>
    </div>
</x-app-layout>
